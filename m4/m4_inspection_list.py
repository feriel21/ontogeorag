...
def main():
    ...
    ap.add_argument(
        "--kg", required=True, help="Original tiered KG (for source passages)"
    )
    ...
    def write_group(fmd, writer, title, note, items, prefix):
        ...
        for j, d in enumerate(items, 1):
            ...
            fmd.write(
                f"**Blind judge (plausibility, no text)**\n\n"
                f"> {blind.get('reasoning', '(none)')}\n\n"
            )
            fmd.write(
                f"**Evidence judge (textual support)**\n\n"
