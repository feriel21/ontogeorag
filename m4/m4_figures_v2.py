...
def fig_sensitivity(report_path, out_dir):
    ...
    rep = json.loads(
        Path(report_path).expanduser().read_text(encoding="utf-8")
    )
    ...
    bars = ax.bar(
        x, sens, width=0.62, color=OI["blue"], edgecolor="white", lw=0.5
    )
    ax.errorbar(
        x,
        sens,
...
def fig_direction(summary_path, out_dir):
    ...
    rep = json.loads(
        Path(summary_path).expanduser().read_text(encoding="utf-8")
    )
    verdicts = rep["verdicts"]
    n = rep["n_checked"]
    order = [
        v
        for v in ("FORWARD", "REVERSE", "UNDIRECTED", "ABSENT", "UNPARSEABLE")
        if v in verdicts
    ]
...
def fig_panel(panel_path, out_dir):
    ...
    rep = json.loads(Path(panel_path).expanduser().read_text(encoding="utf-8"))
    names = rep["judges"]
        ...
        for yi, (val, l) in enumerate(zip(vals, left)):
            if val > 6:
                ax.text(
                    ...
                    va="center",
                    fontsize=8,
                    color="black" if v == "PARTIALLY_SUPPORTED" else "white",
                )
        left += vals
    ax.set_yticks(y)
    ...
    labels = ["Evid.\nunw.", "Evid.\nlinear", "Dec.\nunw.", "Dec.\nlinear"]
    values = [
        k_ev["kappa_unweighted"],
        k_ev["kappa_linear"],
        k_dec["kappa_unweighted"],
        k_dec["kappa_linear"],
    ]

    href = rep.get("human_reference", {}).get("inter_expert_kappa_section_4_4")
    if href:
        axk.axhspan(
            min(href),
            max(href),
    ...
    axk.bar(xk, values, width=0.6, color=OI["blue"], edgecolor="white", lw=0.5)
    for xi, v in zip(xk, values):
        axk.text(
            xi, v + 0.012, f"{v:.2f}", ha="center", va="bottom", fontsize=7
        )
    axk.set_xticks(xk)
