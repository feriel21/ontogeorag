...
def hybrid_retrieve(
    ...
    # --- Dense candidates ---
    q_emb = dense_model.encode([query], normalize_embeddings=True).astype(
        np.float32
    )
    cos_scores = (dense_embeddings @ q_emb.T).squeeze()
    dense_top_idx = np.argsort(cos_scores)[-dense_top_k:][::-1]
    dense_map = {
        chunks[i]["chunk_id"]: float(cos_scores[i]) for i in dense_top_idx
    }
    ...
    for cid in candidate_ids:
        bm25_rank = (
            bm25_ranked.index(cid) + 1 if cid in bm25_map else bm25_top_k + 1
        )
        dense_rank = (
    ...
    pairs = [
        (query, chunk_map[cid]["text"])
        for cid in top_fused
        if cid in chunk_map
    ]
