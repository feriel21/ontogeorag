...
def draw_box(
    ax, xc, yc, w, h, color, label, sublabel, title_fs=14, sub_fs=10.5
):
...
def draw_bold_arrow(
    ax, x0, y0, x1, y1, color="#000000", lw=3.0, ms=25, rad=0.0
):
...
ax.text(
    ...
    fontweight="bold",
    color=C_RETRIEVAL,
    bbox=dict(boxstyle="round,pad=0.2", fc="white", ec=C_RETRIEVAL, lw=2),
    zorder=5,
)
...
ax.text(
    8.5,
    9.2,
    "OntoGeoRAG Pipeline Architecture",
    ha="center",
    va="center",
    fontsize=24,
    fontweight="black",
...
ax.text(
    4.2,
    8.6,
    "LLM-BM25: word-matching search (top-5 direct)",
    ha="center",
    fontsize=11,
    fontweight="bold",
    color="#444444",
...
ax.text(
    12.6,
    8.6,
    "LLM-Rerank: word-matching + semantic re-ranking (top-20 → top-5)",
    ha="center",
    fontsize=11,
    fontweight="bold",
    color="#444444",
...
# Footnote Summary
footer = (
    "The 41 papers are split into normalized text windows and queried using 249 ontology-derived questions.\n"
...
plt.savefig(
    "ontogeorag_pipeline_v3.png", dpi=300, bbox_inches="tight", facecolor=BG
)
