...
def retrieve_passages(
    chunks, subj, obj, rel, reranker=None, bm25_topn=20, top_k=5
):
    ...
    if reranker is not None:
        ...
        top_bm25 = sorted(
            top_bm25, key=lambda i: ce_scores[top_bm25.index(i)], reverse=True
        )
...
def classify_failure(
    retrieved_indices, relevant_indices, extraction_output, subj, rel, obj
):
    """
    ...
    for t in extraction_output:
        if (
            normalize(t.get("subject", "")) == subj_n
            and normalize(t.get("object", "")) == obj_n
        ):
...
def main():
    ...
    for edge in failures:
        ...
        retrieved = retrieve_passages(
            chunks, subj, obj, rel, reranker=reranker, bm25_topn=20, top_k=5
        )
        ...
        for j, (idx, text) in enumerate(zip(retrieved, retrieved_texts)):
            relevant_flag = (
                "RELEVANT" if idx in edge["relevant_chunks"] else "irrelevant"
            )
            print(
                f"  [{j + 1}] chunk_{idx} ({relevant_flag}): {text[:120]}..."
            )

        # Classify
        failure_mode = classify_failure(
            retrieved,
            edge["relevant_chunks"],
