...
data_sorted = sorted(
    data, key=lambda x: (ORDER.get(x["outcome"], 3), -x["corpus_chunks"])
)
...
# ── Y-axis labels ─────────────────────────────────────────────────────
ax.set_yticks(y)
ax.set_yticklabels(labels, fontsize=9.5, fontweight="bold", color="#000000")

# ── Divider lines between groups ─────────────────────────────────────
gap_end = sum(1 for e in data_sorted if e["outcome"] == "CORPUS_GAP")
ret_end = gap_end + sum(
    1 for e in data_sorted if e["outcome"] == "PIPELINE_FAILURE_RETRIEVAL"
)
ext_end = ret_end + sum(
    1 for e in data_sorted if e["outcome"] == "PIPELINE_FAILURE_EXTRACTION"
)

for boundary in [gap_end - 0.5, ret_end - 0.5, ext_end - 0.5]:
    ax.axhline(boundary, color="#000000", lw=1.5, ls="--", zorder=2)
...
sections = [
    (gap_end / 2 - 0.5, "CORPUS\nGAPS\n(n=2)", COLORS["CORPUS_GAP"]),
    (
...
for ypos, txt, col in sections:
    ax.text(right_x, ypos, txt, color=col, **section_style)
...
plt.tight_layout()
plt.savefig(
    "figures/fig_corpus_diagnostic.pdf",
    dpi=300,
    bbox_inches="tight",
    facecolor="#FFFFFF",
)
plt.savefig(
    "figures/fig_corpus_diagnostic.png",
    dpi=300,
    bbox_inches="tight",
    facecolor="#FFFFFF",
)
