...
def find_evidence(triple, chunks, max_chunks=3):
    ...
    stored = (
        triple.get("evidence")
        or triple.get("evidence_quote")
        or triple.get("supporting_sentence", "")
    )
...
def main():
    ...
    for i, triple in enumerate(sampled):
        ...
        if not evidence:
            ...
            raw_output = ""
            log.warning(
                f"  [{i + 1}] No evidence found for ({subj}, {rel}, {obj})"
            )
        else:
        ...
        agree = (
            verdict in ("STRONG_SUPPORT", "WEAK_SUPPORT")
            and q_verdict in ("STRONG_SUPPORT", "WEAK_SUPPORT")
    ...
    agree_count = sum(
        ...
        for r in results
        if (
            r["verdict_llama"] in ("STRONG_SUPPORT", "WEAK_SUPPORT")
            and r["verdict_qwen"] in ("STRONG_SUPPORT", "WEAK_SUPPORT")
        )
    ...
    if ns_rate < 5.0:
        ...
        report_lines.append(f"  Revise Tier-1 reliability claim accordingly.")
