...
def classify(name):
    ...
    if n in SETTINGS:
        return "Setting"
    if "margin" in n or "slope" in n or "setting" in n:
        return "Setting"
    if any(
        p in n
        for p in [
...
for name in node_list:
    ...
    ellipse = mpatches.Ellipse(
        (x, y), width=w, height=h, facecolor=color, edgecolor="none", zorder=3
    )
...
# ── Legends ───────────────────────────────────────────────────────────
node_patches = [
    mpatches.Patch(facecolor=NODE_COLORS[t], edgecolor="none", label=t)
...
edge_patches = [
    ...
    for r in ["hasDescriptor", "occursIn", "causes", "triggers", "partOf"]
]
...
plt.tight_layout()
plt.savefig(
    "figures/fig_kg_subgraph.pdf",
    dpi=300,
    bbox_inches="tight",
    facecolor="#FFFFFF",
)
plt.savefig(
    "figures/fig_kg_subgraph.png",
    dpi=300,
    bbox_inches="tight",
    facecolor="#FFFFFF",
)
