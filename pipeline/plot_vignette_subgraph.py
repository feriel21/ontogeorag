...
for key, (ntype, label) in nodes.items():
    ...
    ring = plt.Circle((x, y), NODE_R + 0.005, color="#000000", zorder=3)
    ax.add_patch(ring)

    circle = plt.Circle((x, y), NODE_R, color=color, zorder=4)
    ax.add_patch(circle)
...
# ── Dividers ──────────────────────────────────────────────────────────
for xv in [0.28, 0.65]:
    ax.axvline(xv, color="#AAAAAA", lw=1.2, ls="--", zorder=0)
ax.axhline(0.14, color="#AAAAAA", lw=1.2, ls="--", zorder=0)

# ── Legend ────────────────────────────────────────────────────────────
patches = [
    mpatches.Patch(
        facecolor=COLORS[t], edgecolor="#000000", linewidth=1.2, label=lbl
    )
    for t, lbl in [
...
plt.tight_layout()
plt.savefig(
    "figures/fig_vignette_subgraph.pdf",
    dpi=300,
    bbox_inches="tight",
    facecolor=BG,
)
plt.savefig(
    "figures/fig_vignette_subgraph.png",
    dpi=300,
    bbox_inches="tight",
    facecolor=BG,
)
