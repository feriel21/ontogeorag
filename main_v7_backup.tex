...
%  - C10 = BM25 + CrossEncoder reranking (153 triples, recall 69.2%)
...
50\% to 69\% against the benchmark, recovering five relation types
...
\begin{figure}[!t]
  ...
  under two configurations: \textbf{C9} passes the
  five highest BM25-scoring chunks directly to the
  extractor; \textbf{C10} first retrieves twenty
  ...
  The effect of the C9-to-C10 re-ranking step on
...
The evaluation benchmark consists of 26 reference edges
...
from the benchmark.
The resulting 26-edge benchmark is the quantitative target
...
experiments.
In the \textbf{BM25-only} configuration (C9), the five
highest BM25-scoring chunks are passed directly to the
extractor.
In the \textbf{re-ranking} configuration (C10), the
...
The \textbf{rescue strategy} is a small set of queries added
...
Results from rescue queries are included in all reported
...
the re-ranking configuration (C10) to re-score BM25 candidate
...
C9; re-ranking C10) are defined in Section~\ref{sec:corpus}.
...
fraction of the 26 benchmark edges from LB2019 that are
...
that retrieval accounts for a net gain of $+34.6$ percentage
...
architectural transitions: the initial rule-based baseline (C1),
the first language-model configuration with verification (C4),
a configuration optimized for precision at the cost of recall (C5),
and the final configuration with re-ranking (C10).
The complete trajectory across all ten configurations is reported
in Appendix~\ref{app:full-trajectory}.

To clarify the naming used throughout: C9 refers to the final
BM25-only configuration, which uses the same architecture as C10
but without the re-ranking step.
C10 refers to the configuration that adds cross-encoder re-ranking
after the BM25 retrieval step.
...
\caption{Comparison of four key pipeline configurations.
  C1: rule-based subject--verb--object extraction (lower bound).
  C4: first language-model configuration, with verification.
  C5: high-precision configuration (verification tightened,
  recall reduced).
  C9: final BM25-only configuration (no re-ranking).
  C10: final configuration with cross-encoder re-ranking.
  The column for C4 shows raw recall before correcting for
...
Moving from rule-based extraction (C1) to language-model extraction
(C4) eliminates the problem of generic \emph{relatedTo} relations,
which accounted for 29--55\% of rule-based output and carry no
geological information.
However, C4 achieves its 65.4\% recall at a 78\% not-supported
rate  meaning nearly four out of five matched relations are
not actually supported by the source text.
Correcting for this, the effective reliable recall of C4 is
...
C5 demonstrates that the not-supported rate can be brought to
...
C9 recovers recall through dual-pass extraction and a larger
query set, reaching 50.0\% at near-zero not-supported rate.
C10 adds cross-encoder re-ranking on top of C9 and reaches
69.2\%, as discussed in detail in Section~\ref{sec:run11}.
%==========================
%--
...
step (C10) raises recall from 50.0\% to 69.2\%,
recovering five benchmark edges that BM25 alone had
consistently missed across all prior configurations.
Figure~\ref{fig:retrieval-comparison} shows the
progression from memory-only to C9 to C10 in terms
of both recall and silent query rate.

The five newly recovered edges all share the same
retrieval failure mode in C9: BM25 retrieves passages
...
The five relation types recovered exclusively by C10
...
The gain of +19.2 percentage points from C9 to C10
...
\textbf{Configuration}
  ...
  & $H_{T1}$ (\%) \\
\midrule
C4 (first LLM) & 78.0 (71.3--83.7) & 83.1 & 78.0 \\
C5 (high-prec.) & 22.8 (14.5--33.5) & 29.9 & 0.0 (0.0--14.3) \\
C10 Tier~1 & \textbf{0.0} (\WI{0.0}{3.6}) & 0.0 & \textbf{0.0} \\
C10 Tier~1+2 & \textbf{2.9} (\WI{0.9}{7.5}) & 2.9 & 0.0 \\
\bottomrule
\end{tabular}
\end{table}

From C4 to C10, unsupported generation decreases substantially through
the combined effect of ontology constraints, retrieval grounding,
verification, and post-extraction filtering.
The final Tier-1 graph achieves 0\% NOT\_SUPPORTED under the verifier
protocol, which should be interpreted as a pipeline-internal support
result rather than as a guarantee of factual correctness.
...
Table~\ref{tab:final-kg} summarizes the final reranked C10 knowledge
graph at the two operational levels used in this paper.

\begin{table}[htb]
\centering
\caption{Final tiered KG (C10): properties at each operating point.}
\label{tab:final-kg}
\begin{tabular}{lcc}
\toprule
...
\midrule
Triples & 101 & 153 \\
LB2019 recall & 8/26 (30.8\%) & 18/26 (69.2\%) \\
...
The final pipeline recovers 18 of the 26 reference
...
inspected the top-5 passages retrieved by C10 and
...
in the C10 top-5: \pred{debris flow hasDescriptor
...
Six edges have at least one relevant chunk in the C10
...
\begin{table}[htb]
\centering
\caption{Recovery of the 26-edge benchmark by relation
  type, comparing C9 (BM25-only) and C10 (with
...
\begin{figure}[htb]
  ...
  Outcome markers: dark green = recovered by C10;
  ...
  gap (evidence absent).
  Labels C9 and C10 indicate which configurations
...
\begin{figure}[htb]
  ...
  \textbf{C9}: BM25 keyword retrieval only.
  \textbf{C10}: BM25 retrieval followed by
  cross-encoder re-ranking.
  The dashed orange line on the left panel shows the
  corpus-gap corrected recall ceiling of 75.0\%
  (18 of 24 recoverable edges), which accounts for
...
Without any retrieval, the model recovers 9 of the 26 reference
edges (34.6\%) from parametric memory alone, producing 53 triples
from 249 queries, with 88\% of queries producing no output.
With BM25 retrieval (C9), recall rises to 13/26 = 50.0\%,
with 58.2\% of queries producing no output.
With re-ranking added (C10), recall reaches 18/26 = 69.2\%.
The progression from memory-only to full pipeline
represents a net gain of $+34.6$ percentage points
...
58.2\% (C9) to 39.4\% (C10), a total reduction of
...
directly explaining the recall gain from C9 to C10.
...
\caption{Effect of retrieval on recall, comparing the
  ...
  retrieval (C9), and retrieval with re-ranking (C10).
...
\midrule
Memory only  & 34.6 (9/26)  & 88.0 &  53 \\
C9 (BM25)  & 50.0 (13/26) & 58.2 & 103 \\
C10 (rerank) & 69.2 (18/26) & \textbf{39.4} & 153 \\
\midrule
Gain C9 vs memory  & +15.4 pp & $-$29.8 pp & \\
Gain C10 vs memory & +34.6 pp & $-$48.6 pp & \\
\bottomrule
\end{tabular}
\end{table}
...
The 26-edge benchmark is a subset of LB2019, restricted to edges
...
Recall against the 26 reference edges should therefore be read as a
...
The final configuration recovers 18 of 26 benchmark relations
(69.2\%), produces a graph where the highest-confidence triples are
...
A recall of 69.2\% against the 26-edge benchmark sounds like a
...
achievable recall is 24/26 = 92.3\%, not 100\%.
Correcting for these corpus gaps, the pipeline recovers 18 of 24
recoverable edges (75.0\%).
...
For these reasons, 69.2\% should be read as a lower bound on
...
of the final C10 graph and to characterize remaining error modes
...
Qwen-7B verification protocol (Wilson CI: \WI{0.0}{3.6}).
...
containing 153 triples (101 in Tier-1) and achieves 69.2\% recall
against the 26-edge LB2019 benchmark, with coverage of 10 of the
...
relations.
Introducing CrossEncoder reranking increased recall from 50.0\% to
69.2\% and expanded the final graph from 103 to 153 triples while
preserving 0\% NOT\_SUPPORTED in Tier-1 under the verifier protocol.
...
(C1, C4, C5, C9, C10) are shown in bold.
...
\caption{Complete pipeline configuration trajectory.
  ...
  Recall against the 26-edge LB2019 benchmark.
...
\textbf{C9}
  & Final BM25-only
  & No & normal & 534 & 103 & 50.0 & 11/13 & 0.0 \\
\textbf{C10}
  & Add CrossEncoder rerank
  & Yes & normal & 745 & 153 & 69.2 & 10/13 & \textbf{0.0} \\
\bottomrule
\end{tabular}
\end{table*}
...
Configurations C1--C7 are developmental iterations;
C8, C9, and C10 are the three fully instrumented
...
\caption{Complete pipeline configuration trajectory.
  ...
  \emph{Recall} = fraction of 26-edge LB2019
...
\textbf{C9}
  & Expand query set to 249 (rescue strategy)
  & No & 189/186 & 71/61 & 9/26 & 13/26 (50.0\%) & 10/13 & 0.0 \\
C10$^*$
  & Intermediate reranking test
  & Yes & 267/248 & 82/81 & 11--12/26 & 12/26 (46.2\%) & 11/13 & 0.0 \\
\textbf{C10}
  & Final CrossEncoder reranking
  & Yes & 380/365 & 115/117 & 17/26 & 18/26 (69.2\%) & 10/13 & \textbf{0.0} \\
\bottomrule
\end{tabular}
...
completeness. The key transitions captured by C1,
C4, C5, C9, and C10 represent the main evaluation
...
context: 41).
C9 expanded this to 249 queries by adding a
fourth rescue strategy and increasing coverage
