#!/bin/bash
#SBATCH --job-name=run10_ontogeorag
...
#SBATCH --output=/home/talbi/ontogeorag/logs/run10_%j.out
#SBATCH --error=/home/talbi/ontogeorag/logs/run10_%j.err
...
A=$REPO/output/run10_a
B=$REPO/output/run10_b
KG=$REPO/output/run10_kg
...
echo "============================================"
echo " OntoGeoRAG Run 10 (GPU) — $(date)"
echo " Node: $(hostname)"
echo " GPU:  $(nvidia-smi --query-gpu=name --format=csv,noheader 2>/dev/null)"
echo " P6: descriptor_partial -> normalize"
echo " P8: reinforced non-MTD subject extraction"
echo " Fusion: run10_a + run10_b only (no v4/v7 cascade)"
echo "============================================"
...
python -u pipeline/07_final_metrics.py \
    --kg     $KG/tiered_kg_run10.json \
    --output $KG/metrics_run10.json
...
kg = json.load(open('$KG/tiered_kg_run10.json'))
