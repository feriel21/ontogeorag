#!/bin/bash
#SBATCH --job-name=run8_ontogeorag
#SBATCH --partition=convergence
#SBATCH --mem=48G
#SBATCH --time=24:00:00
#SBATCH --cpus-per-task=8
#SBATCH --output=/home/talbi/ontogeorag/logs/run8_%j.out
#SBATCH --error=/home/talbi/ontogeorag/logs/run8_%j.err
...
A=$REPO/output/run8_a
B=$REPO/output/run8_b
KG=$REPO/output/run8_kg
...
echo "============================================"
echo " OntoGeoRAG Run 8 (GPU) — $(date)"
echo " Node: $(hostname)"
echo " GPU:  $(nvidia-smi --query-gpu=name --format=csv,noheader 2>/dev/null)"
echo " P1: extended RELATION_MAP (0 triples lost)"
echo " P2: few-shot gold examples in prompts"
echo " P3: evidence aggregation in fusion"
echo "============================================"

# ── PASS A (temp=0.0 — deterministic) ─────────────
echo ""
...
kg = json.load(open('$KG/tiered_kg_run8.json'))
...
halluc  = [t for t in triples if t.get('verdict','')=='NOT_SUPPORTED']
covered = set()
for t in triples:
...
print(f'  Hallucination      : {len(halluc)/len(triples)*100:.1f}%')
