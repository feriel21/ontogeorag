...
def load_kg(path: str) -> list[dict]:
    """Load KG JSON, handling all known OntoGeoRAG formats."""
    raw = json.loads(Path(path).read_text())

    if isinstance(raw, dict):
        # Format A: m4_integrate_tiers output — {meta, tier1, tier2, quarantine?}
        if "tier1" in raw or "tier2" in raw:
            triples = []
            for t in raw.get("tier1", []):
                triples.append(t)
            for t in raw.get("tier2", []):
                triples.append(t)
            for t in raw.get("quarantine", []):
                t_copy = dict(t)
                t_copy["quarantine"] = True
                triples.append(t_copy)
            print(f"  Format: m4_integrate_tiers (tier1={len(raw.get('tier1',[]))}, "
                  f"tier2={len(raw.get('tier2',[]))}, "
                  f"quarantine={len(raw.get('quarantine',[]))})")
            return triples

        # Format B: 06_tiered_fusion output — {metadata, triples}
        if "triples" in raw:
            return raw["triples"]

    # Format C: list-of-pairs [[key, value], ...]
    if isinstance(raw, list) and len(raw) > 0 and isinstance(raw[0], list):
        d = dict(raw)
        return d.get("triples", [])

    # Format D: direct list of triple dicts
...
def kg_portrait(triples: list[dict], outdir: Path):
    ...
    if not active:
        print("  ERROR: 0 active triples — check KG format / loader.")
        return [], {}, {}
