...
def main():
    ...
    missing = [name for name, data in
               [("graph_statistics", graph), ("node_statistics", nodes),
                ("relation_statistics", rels),
                ("descriptor_statistics", descs),
                ("provenance_report", prov)] if not data]
    if missing:
        raise SystemExit(f"Missing upstream outputs: {missing}. "
    ...
    kr.append("|---|---|---|")
    for r in read_csv(d / "graph_statistics.csv"):
        kr.append(f"| {r['metric']} | {r['value']} | "
                  f"{r['report_in_main_text']} |")
    kr += ["", "## Files",
    ...
    disc = ["# Discussion (AUTO-DRAFT — scaffolding to be rewritten by the "
    ...
    disc.append(
        ...
        + ", ".join(f"**{r['node']}** (degree {r['degree']}, "
                    f"{r['n_papers']} papers)" for r in hubs)
    ...
    causal = [r for r in prov if r["relation"].lower() in
    ...
    if causal:
        ...
        links = [f"*{r['subject']}* —{r['relation']}→ *{r['object']}* "
                 f"(T{r['tier']}, {r['support_papers']} papers)"
                 for r in sorted(causal,
                                 key=lambda x: -int(x["support_papers"]))[:8]]
        disc.append("Causal assertions present in the graph: "
                    + "; ".join(links) + ". [provenance_report.csv] "
        ...
        disc.append("No causal triples present — the mechanism layer is "
                    "empty in this run. [provenance_report.csv]")

    controls = [r for r in prov if r["relation"].lower() == "controls"]
    disc.append("\n## Geological controls")
    disc.append((f"{len(controls)} `controls` assertions. " if controls
    ...
    if envs:
        ...
        disc.append("Depositional settings asserted: "
    ...
    if mtd_rows:
        ...
        disc.append("No descriptor row maps to an MTD-like object — check "
                    "canonicalization of the MTD node.")

    disc.append("\n## Under-documented concepts")
    weak = [r for r in vocab if int(r["n_papers"] or 0) <= 1]
    disc.append(f"{len(weak)} concepts are supported by ≤1 paper "
    ...
    disc.append("Highest-betweenness nodes: "
