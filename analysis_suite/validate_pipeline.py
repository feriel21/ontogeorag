...
def run(stage, cmd, timeout=900, env_extra=None, reco_on_fail=""):
    ...
    try:
        ...
        record(
            stage, "FAIL", f"timeout {timeout}s", str(logfile), reco_on_fail
        )
    return False
...
def check_protocol():
    ...
    record(
        ...
        "OK" if ok else "FAIL",
        "dev/test/declaration committed" if ok else "protocol files missing",
    )
