...
def main():
    ...
    for t in active:
        ...
        for k in ("source_type", "target_type", "subject_type", "object_type"):
            if t.get(k):
                types.add(str(t[k]))
    ...
    try:
        communities = list(nx.community.greedy_modularity_communities(largest))
    ...
    stats = [
        ...
        (
            "modularity",
            round(modularity, 3) if modularity == modularity else "nan",
            "annex",
        ),
        ...
        (
            ...
            round(safe(nx.average_shortest_path_length, largest), 2),
            "annex",
        ),
        ("avg_clustering", round(safe(nx.average_clustering, U), 3), "annex"),
        (
            "degree_assortativity",
            round(safe(nx.degree_assortativity_coefficient, U), 3),
            "annex",
        ),
        (
            "n_cycles_directed",
            len(list(nx.simple_cycles(D)))
            if D.number_of_nodes() < 500
            else "skipped",
            "annex",
        ),
    ]
    with open(
        outdir / "graph_statistics.csv", "w", newline="", encoding="utf-8"
    ) as f:
        w = csv.writer(f)
    ...
    with open(
        outdir / "node_statistics.csv", "w", newline="", encoding="utf-8"
    ) as f:
        w = csv.writer(f)
        w.writerow(
            [
    ...
    nx.draw_networkx_nodes(
        U, pos, ax=ax, node_color=colors, node_size=sizes, alpha=0.9
    )
    labels = {n: n for n in U if deg[n] >= max(2, np.percentile(degs, 80))}
    nx.draw_networkx_labels(
        U, pos, labels, ax=ax, font_size=8, font_family="DejaVu Sans"
    )
    ...
    ax.barh(range(len(topN))[::-1], [deg[n] for n in topN], color=TERRACOTTA)
    ...
    lines.append(
        "- **Authorities (in-degree — typically descriptors/"
        "settings)**: "
    ...
    if btw:
        ...
        lines.append(
            "- **Mechanistic pivots (betweenness)**: "
    ...
    lines.append(
        f"- **Peripheral concepts (degree ≤1)**: {len(isolated)} — "
    ...
    print(
        f"[12] graph: {U.number_of_nodes()} nodes / "
